...
async function makeBackendRequest(endpoint: string, data: ProfileData, req: Request) {
    try {
    ...
    try {
        ...
        if (!current || !original) {
            return NextResponse.json({
                success: false,
                statusCode: 400,
                message: 'Invalid request body. Must include current and original data'
            }, { status: 400 });
        }
        ...
        const promises = Object.entries(changedFields).map(async ([field, config]) => {
            const result = await makeBackendRequest(config.endpoint, config.data, req);
        ...
        return NextResponse.json({
            success: false,
            statusCode: 500,
            message: error instanceof Error ? error.message : 'Failed to change profile'
        }, { status: 500 });
    }
}
